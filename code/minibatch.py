...
def start_main(par, param_obj, path_obj):
    ...
    if par.metodo_classificazione_celle == 1:
        ...
        print(cells_th1, param_obj.division_threshold)
        result = lay.classification_surface(vertices, cells_th1, param_obj.division_threshold)
        if len(result) == 8:
            (cells_th1, cells_out_th1, cells_polygons_th1, indexes_th1, cells_partials_th1, contour_th1, centroid_th1, points_th1) = result
            print(f"centroid_th1: {centroid_th1}")
            print(f"cells_polygons_th1: {len(cells_polygons_th1)} polygons")
            print(f"cells_out_th1: {len(cells_out_th1)} ")
        else:
            print(f"Unexpected result from classification_surface: {result}")
    # -------------------------------------------------------------------------------------
